...
// Componente para cada fila ordenable
const SortableItem = ({ item, editingId, startEdit, onDeleteItem, handleToggleCheck, children }) => {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    // Note: Temporarily removed Swipe-to-Delete to resolve mobile stability issues.
...
const MainPanel = ({ list, onAddItem, onDeleteItem, onUpdateItem, onUpdateList }) => {
    ...
    // Auto-scroll al final cuando se agrega un nuevo producto
    useEffect(() => {
    ...
    const calculateTotal = () => {
        // Solo suma productos NO completados
        return list.items
    ...
    const calculateSavings = () => {
        // Lo que ya "ahorraste" (productos marcados)
        return list.items
            .filter(item => item.checked)
            .reduce((sum, item) => sum + (parseFloat(item.price) || 0), 0);
    };
    ...
    const handleToggleCheck = (itemId, currentStatus) => {
        ...
        // Trigger confetti if all items are checked
        if (newStatus) {
        ...
        <div className="main-panel">
            ...
            <div className="panel-header-section">
                <div className="header-titles">
                    <h1 className="main-title">{list.name}</h1>
            ...
            <div className="add-product-container">
                ...
                <form className="add-form" onSubmit={handleSubmit}>
                    ...
                    <label className="form-group flex-wide">
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                // Smart Check: If user selects an autocomplete item, fill category/price
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {editingId && (
                            <button type="button" onClick={cancelEdit} className="btn-cancel" title="Cancelar edición">
                                <span className="material-symbols-outlined">close</span>
                            </button>
                        )}
                        <button type="submit" className="btn-add" title={editingId ? 'Guardar cambios' : 'Agregar producto'}>
